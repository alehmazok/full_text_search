...
void main() {
  ...
  final searchables = [one, two, three, four];
  ...
  test("When isMatchAll = true, natural sorting is retained", () async {
    final List<Searchable> matches = await FullTextSearch<Searchable>(
      ...
      items: searchables,
      isMatchAll: true,
      tokenize: (s) => [s.name, s.phone, s.address],
    ).findResults();
    expect(matches, containsAll([one, two, three]));
  });

  test("When isMatchAll = false, we sort by numTokens correctly", () async {
    final List<TermSearchResult<Searchable>> matches2 = await FullTextSearch<Searchable>(
      term: "John",
      items: searchables,
      isMatchAll: false,
      isStartsWith: false,
    ...
    expect(matches2.map((it) => it.result), containsAllInOrder([two, one, three]));
  });
  ...
  test("When isMatchAll = false, applies proper weights for partial matches (2 term)", () async {
  ...
  test("When isMatchAll = false, applies proper weights for partial matches", () async {
    final matches = await FullTextSearch<Searchable>(
      term: "Mac",
      items: [bigMacGruber, macdonaldDouglas, mac],
      isMatchAll: false,
